...
# here execute ldconfig to regenerate /etc/ldcache to add nvidia libraries
# so that gpu isolator can find out the needed libaray, and copy them to 
# /var/run/mesos/isolators/gpu/nvidia_<version>/lib.
ldconfig
