...
import (
	...
	"linkernetworks.com/dcos-backend/client/api/documents"
...
var (
	...
	PropertiesFile = flag.String("config", "dcos_client.properties", "the configuration file")
...
func main() {
	...
	dao.DAO = &dao.Dao{
		SessMng:    sessMng,
		MongoAlias: MongoAlias,
	}
	fmt.Println("mongodb.alias is " + dao.DAO.MongoAlias)
	...
	config.URL = "http://" + Props.GetString("marathon.endpoint", "master.mesos:8080")
	...
	// no need to access body more than once (method has been removed)
	// restful.SetCacheReadEntity(false)
	// API Cross-origin requests
	apiCors := Props.GetBool("http.server.cors", false)
	logrus.Debugf("CORS is enabled? %v", apiCors)
	// Documents API
	...
	logrus.Infof("endpoint is %v", endpoint)
	// Serve favicon.ico
	http.HandleFunc("/favion.ico", icon)

	basePath := Props.MustGet("http.server.host") + ":" + Props.MustGet("http.server.port")
	logrus.Infof("basePath is %v", basePath)

	isHttpsEnabled := Props.GetBool("http.server.https.enabled", false)
	caCertPath := Props.GetString("http.dcosdeploy.https.crt", "")
	logrus.Debugf("httpEnabled=%v", isHttpsEnabled)
	common.Logger = utils.GetInstance(isHttpsEnabled, caCertPath)
	logrus.Infof("logger is %v", common.Logger)
	common.BasicInfo, _ = common.Logger.GetBasicInfo()
	common.MgmtIps = common.BasicInfo.MgmtIp
	common.Url = common.Logger.CreateLogUrl(common.MgmtIps)
	if isHttpsEnabled {
		// Register Swagger UI
...
func init() {
	...
	logrus.Infof("PropertiesFile is %s\n", *PropertiesFile)
	var err error
	...
	// logFile := Props.GetString("logrus.file", "/var/log/linkerdcos_client.log")
	// f, err := os.OpenFile(logFile, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0777)
	// if err != nil {
	// 	fmt.Printf("error opening file %v", err)
	// 	f, err = os.OpenFile("linkerdcos_client.log", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0777)
	// 	if err != nil {
	// 		fmt.Printf("still failed to open log file linkerdcos_client.log %v", err)
	// 	}
	// }
	// logrus.SetOutput(f)
