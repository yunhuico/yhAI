...
func TestPromRulesMarshal(t *testing.T) {
	...
	expectData03 := readFile(t, "expect-data-03.rules")
	...
	var cases = []struct {
		...
		{[]PromRule{}, []byte{}, nil},
		{[]PromRule{rule1, rule2}, expectData03, nil},
	}
	for _, c := range cases {
		gotData, gotErr := c.PromRules.Marshal()
